...
Сужение типа работает не только для типа `unknown`. Это универсальный механизм, который работает со всеми возможными типами, например, _Union Types_:
...
Компилятор не переносит типы из сигнатур перегрузок в тело функции. Даже если все перегрузки описывают одинаковые типы параметров, внутри реализации мы получим широкий тип и сузить его придется самостоятельно.
