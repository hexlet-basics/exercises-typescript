...
Хотя `func()` возвращает `true`, переменная `result` имеет тип `void`. Это значит, что использовать ее можно только для присваивания другой переменной с типом `void`.
