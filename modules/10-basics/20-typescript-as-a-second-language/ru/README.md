TypeScript на нашем проекте дается как второй язык. Поэтому чтобы лучше понимать материал курса, стоит владеть JavaScript или другими языками программирования.

Также нужно иметь представление о типах данных, переменных, условных конструкциях, циклах, функциях, свойствах и методах объектов, а также анонимных или лямбда-функциях.
...
> _The right-hand side of an arithmetic operation must be of type 'any', 'number', 'bigint' or an enum type_
...
Затем нужно создать файлы с кодом на TypeScript. Они имеют расширение _ts_. Компилятор TypeScript доступен через утилиту `tsc`:
